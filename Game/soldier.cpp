...
bool Soldier::shot(std::list<Bullet> &bullets, sf::FloatRect hero_rec){
    ...
    if(rec.intersects(hero_rec) && hp_ > 0 && shot_clock_.getElapsedTime().asSeconds() >= 0.4){
        ...
        Bullet bullet(last_direction_, barrel_cords_.first, barrel_cords_.second, bullet_textures_);
